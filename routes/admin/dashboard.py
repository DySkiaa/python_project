...
def dashboard():
    ...
    cust = len(cust)
    produk = len(produk)
    total = sum(transaksi.total for transaksi in transaksi if transaksi.status_pembayaran == '3')
...
@dashboard_page.route('/admin/dashboard/change_password/<id>', methods=['GET', 'POST'])
@login_required
